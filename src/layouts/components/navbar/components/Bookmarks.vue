...
export default {
  ...
  computed: {
    navbarSearchAndPinList () {
      return {pages: this.$store.state.layout.navbarSearchAndPinList['pages']};
    },
    starredPages () {
    ...
    textColor () {
      return {'text-white': this.$store.state.layout.mainLayoutType === 'vertical' && this.navbarColor !== (this.$store.state.layout.theme === 'dark' ? '#10163a' : '#fff') };
    }
  },
  methods: {
    selected (obj) {
