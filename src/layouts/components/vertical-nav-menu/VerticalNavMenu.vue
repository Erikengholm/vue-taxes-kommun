<template>
<div class="parentx">
  <vs-sidebar class="v-nav-menu items-no-padding" :class="{menuShrink: !isMouseEnter && reduce}" v-model="isVerticalNavMenuActive" ref="verticalNavMenu" default-index="-1" :click-not-close="clickNotClose" :reduce-not-rebound="reduceNotRebound" :parent="parent" :hiddenBackground="clickNotClose"
      ...
      <div class="header-sidebar flex items-end justify-between" slot="header">
        ...
        <div>
          ...
          <template v-if="showCloseButton">
            <feather-icon icon="XIcon" class="m-0 cursor-pointer" @click="$store.commit('layout/TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', false)" />
          </template>
      ...
      <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" @scroll="psSectionScroll" :key="$vs.rtl">
        <template v-for="(item, index) in menuItemsUpdated">
          ...
          <template v-else-if="!item.header">
            ...
            <!-- Nav-Item -->
            <v-nav-menu-item v-if="!item.submenu" :key="`item-${index}`" :index="index" :to="item.slug !== 'external' ? item.url : null" :href="item.slug === 'external' ? item.url : null" :icon="item.icon" :target="item.target"
...
export default {
  ...
  computed: {
    ...
    isVerticalNavMenuActive: {
      get () {
        return this.$store.state.layout.isVerticalNavMenuActive;
      },
      set (val) {
        this.$store.commit('layout/TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', val);
      }
    },
    layoutType () {
    ...
    reduceButton: {
      ...
      set (val) {
        this.$store.commit('layout/TOGGLE_REDUCE_BUTTON', val);
      }
    },
    ...
    verticalNavMenuItemsMin () {
      return this.$store.state.layout.verticalNavMenuItemsMin;
    },
    scrollbarTag () {
      // console.log(this.$store.getters.scrollbarTag, 'scrollbarTag')
  ...
  watch: {
    '$route' () {
      if (this.isVerticalNavMenuActive && this.showCloseButton) this.$store.commit('layout/TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', false);
    },
    reduce (val) {
      const verticalNavMenuWidth = val ? 'reduced' : this.$store.state.layout.windowWidth < 1200 ? 'no-nav-menu' : 'default';
      this.$store.dispatch('layout/updateVerticalNavMenuWidth', verticalNavMenuWidth);
  ...
  methods: {
    ...
    mouseEnter () {
      if (this.reduce) this.$store.commit('layout/UPDATE_VERTICAL_NAV_MENU_ITEMS_MIN', false);
      this.isMouseEnter = true;
    },
    mouseLeave () {
      if (this.reduce) this.$store.commit('layout/UPDATE_VERTICAL_NAV_MENU_ITEMS_MIN', true);
    ...
    setVerticalNavMenuWidth () {
      ...
      if (this.windowWidth > 1200) {
        if (this.layoutType === 'vertical') {
          ...
          this.$store.commit('layout/UPDATE_VERTICAL_NAV_MENU_ITEMS_MIN', verticalNavMenuItemsMin);
      ...
      this.$store.commit('layout/UPDATE_VERTICAL_NAV_MENU_ITEMS_MIN', false);
